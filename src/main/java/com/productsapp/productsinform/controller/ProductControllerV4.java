...
import com.productsapp.productsinform.service.ProductServiceInterfaceV4;
...
public class ProductControllerV4 {
    ...
    private final ProductServiceInterfaceV4 productService;
    ...
    public PageResponse<ProductDto> getProductByCategory(@PathVariable String category,
                                                         @RequestParam(defaultValue = "0") int page,
                                                         @RequestParam(defaultValue = "10") int size,
                                                         @RequestParam(defaultValue = "id") String sortBy,
                                                         @RequestParam(defaultValue = "asc") String direction) {
    ...
    public ProductDto seenPatch(@PathVariable long id){
        return productService.seenPatch(id);
    }


    @PatchMapping("/paid/{id}")
    public ProductDto paidPatch(@PathVariable long id){
        return productService.paidPatch(id);
    }
}